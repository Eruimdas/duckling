...
rulePrecision = Rule
    ...
    , pattern =
      [ regex "\\~|tam|yaklaşık|aşağı yukarı|yakın|civarında|hemen hemen"
      , dimension Quantity
      ]
      , prod = \case
...
ruleIntervalMax = Rule
    ...
    , pattern =
      [ regex "az|eksik|altında|en fazla|en çok|daha az"
      , Predicate isSimpleQuantity
      ]
    , prod = \case
        (_:
...
ruleIntervalMin = Rule
  ...
  , pattern =
      [ regex "fazla|en az|aşkın|aşan|en az|daha fazla"
      , Predicate isSimpleQuantity
      ]
    , prod = \case
        (_:
